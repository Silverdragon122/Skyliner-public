...
namespace PlayerHelper {
    ...
    void damagePlayerHelper(al::LiveActor *actor) {
        PlayerActorHakoniwa *mainPlayer = (PlayerActorHakoniwa *) al::getPlayerActor(actor, 0);

        GameDataFunction::damagePlayer(GameDataHolderAccessor(actor));

    }

    void damagePlayerHelper(PlayerActorHakoniwa *mainPlayer) {
        GameDataFunction::damagePlayer(GameDataHolderAccessor(mainPlayer));
    }
