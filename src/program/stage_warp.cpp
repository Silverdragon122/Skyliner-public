#include <program/stage_warp.hpp>
...
void drawStageWarpWindow() {
    ...
    ImGui::SetWindowPos(ImVec2(300, 0), ImGuiCond_FirstUseEver);

    ImGui::Begin("Skyliner Warp", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize |
            ...
            for (auto &dbEntry: entry.mStageNames) {
                ...
                if (isInGame) {
                    if (ImGui::MenuItem(getEnglishName(stageName))) {
                        Logger::log("Warping to %s", stageName);
                        isWarpInProgrees = true;
